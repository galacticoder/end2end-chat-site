...
export function createEnqueueOutbound(refs: MessagingRefs) {
  return (to: string, envelope: any, type: SignalType.TEXT | SignalType.TYPING | SignalType.REACTION | SignalType.FILE, ttlMs: number) => {
    const now = Date.now();
...
export function createSendP2PMessage(
  ...
  enqueueOutbound: (to: string, envelope: any, type: SignalType.TEXT | SignalType.TYPING | SignalType.REACTION | SignalType.FILE, ttlMs: number) => void,
  ...
  return async (
    ...
    const isTyping = options?.messageType === SignalType.TYPING;
    ...
    try {
      ...
      let p2pType: SignalType.CHAT | SignalType.TYPING | SignalType.REACTION | SignalType.FILE = SignalType.CHAT;
      ...
      const connectedPeers = refs.p2pServiceRef.current?.getConnectedPeers() ?? [];
      const serviceConnected = connectedPeers.includes(to);
      if (!serviceConnected) {
        const ttl = p2pType === SignalType.TYPING ? 5000 : p2pType === SignalType.REACTION ? 30000 : p2pType === SignalType.FILE ? 180000 : 120000;
      ...
      await refs.p2pServiceRef.current.sendMessage(to, encryptedEnvelope, p2pType);
      ...
      if (errorMsg.includes('no PQ session') || errorMsg.includes('without established PQ session')) {
        try {
          const p2pType: SignalType.CHAT | SignalType.TYPING | SignalType.REACTION | SignalType.FILE =
            options?.messageType === SignalType.TYPING ? SignalType.TYPING
      ...
      try {
        ...
        enqueueOutbound(to, encryptedEnvelope, (options?.messageType === SignalType.TYPING ? SignalType.TYPING : (options?.messageType === SignalType.REACTION ? SignalType.REACTION : SignalType.CHAT)) as any, ttl);
      } catch { }
      setLastError(_error);
      return {
    ...
    try {
      if (message.type === 'signal') {
        try {
          ...
          if (!parsed) {
            const service: any = refs.p2pServiceRef.current as any;
          ...
          if (parsed.kind !== EventType.CALL_SIGNAL) {
            return;
          }
      ...
      if (message.type === SignalType.DELIVERY_ACK) {
        try {
      ...
      let messageType: SignalType.TEXT | SignalType.TYPING | SignalType.REACTION | SignalType.FILE | SignalType.EDIT | SignalType.DELETE = SignalType.TEXT;
      if (message.type === SignalType.TYPING) messageType = SignalType.TYPING;
      else if (message.type === SignalType.REACTION) messageType = SignalType.REACTION;
      ...
      if (message.type === SignalType.TYPING) {
        try {
          window.dispatchEvent(
